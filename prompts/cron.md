...
| `pr-comments-on-mine`      | Doctor's invariant 13 auto-dispatches a fix-comments worker for pr-open work items with no active session, so most of these never reach you as `status=new`. If you do see one, it means either (a) no pr-open work item matches the PR URL — create a work item at priority 2 to address comments; or (b) a worker was already active on that work item — notify urgent so I see reviewer activity on a live session. (cos PRs: same rules — if a fix worker is already dispatched, great; if not, treat normally.)                                                                                                                                                                     |
