...
| `pr-comments-on-mine`      | Check if there's already a pr-open work item for this PR (search `cos fleet --format json` output). **If yes:** redispatch a fix-comments worker with `cos dispatch <wi-id> --force` and suppress the signal — the worker picks up the fix-comments mode automatically because the WI is still pr-open. Also notify normal (not urgent) so I see the activity. **If no:** create a work item at priority 2 to address comments. **cos PRs:** still notify urgent — a comment on a cos PR means I wanted to say something, don't auto-merge past it, and don't auto-redispatch either.                                                                                                    |
