...
| `meeting-prep-feedback`    | The user submitted a note on a meeting prep page ("too long", "missed X", "this was great"). `cos signal-triage <id> notify --urgency normal --title "Prep feedback: <payload.summary or payload.slug>" --body "<payload.feedback truncated to ~200 chars> — event=<payload.event_id>"`. Notify only — do NOT auto-modify the collector prompt. I decide whether the feedback is worth an `enqueue` to tune the prompt.                                                                                                                                                                                                                                                                  |
