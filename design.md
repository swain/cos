...
### Why invariant 8 exists

The operational directory `~/.claude/cos/` symlinks `prompts/`, `bin/`, `cli/`, and `launchd/` into `~/Repos/cos/`. If `~/Repos/cos` is parked on an old worker branch — as happened because of the stale `cmdWorkerSetup` special-case that `cd`'d the primary tree onto a worker's branch — those symlinks serve that branch's content instead of `main`'s. Prompts revert, the built `cli/dist/index.js` reverts, and debugging is brutal because everything _looks_ right. The invariant catches this on every tick and re-pins the tree to `origin/main` unless there's uncommitted work to preserve.
